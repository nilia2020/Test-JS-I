...
function dePalabrasAFrase(palabras) {
  ...
  // Tu código:
  let frase = "";
  palabras.map((elemento) => (frase += elemento + " "));
  return frase.trim();
}
...
function promedioResultadosTest(resultadosTest) {
  ...
  // Tu código:
  return (
    resultadosTest.reduce((acumulador, elemento) => acumulador + elemento) /
...
function cuentoElementos(arreglo) {
  ...
  return arreglo.filter((elemento) => elemento > 19).length;
}
...
function todosIguales(arreglo) {
  ...
  //Escribe tu código aquí
  return arreglo.reduce((acumulador, elemento) => acumulador + elemento) /
    arreglo.length ===
    arreglo[0]
    ? true
    : false;
